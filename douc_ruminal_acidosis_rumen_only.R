...
map = read.delim('data/douc_ruminal_acidosis_rumen_only_meta-analysis_merged_mapping.txt', row.names = 1) # Grab the map
# map$BodysiteSpeciesSpecific = factor(levels = c("Douc Foregut","Cow Rumen Control", "Cow Rumen APSC", "Cow Rumen GBSC"),ordered = T)

### Laod and Filter OTU and Taxa tables ###
taxa = read.delim('data/douc_ruminal_acidosis_rumen_only_meta-analysis_taxatable.txt',row=1,as.is=T)
taxa = taxa[,rownames(map)]  # sync the sample names for the Taxa table
otu = read.delim('data/douc_ruminal_acidosis_rumen_only_meta-analysis_otutable.txt',row=1,as.is=T)
...
pdf("results/douc_ruminal_acidosis/rumen_only/bray_bodysitespeciesspecific_rumenonly.pdf",width=6,height=4.75); plot_pcoa(bray,map,category='BodysiteSpeciesSpecific');
pdf("results/douc_ruminal_acidosis/rumen_only/uuf_bodysitespeciesspecific_rumenonly.pdf",width=6,height=4.75); pcoa.u = plot_unifrac(otu.s,map,tree,category='BodysiteSpeciesSpecific',weight=F); 
pdf("results/douc_ruminal_acidosis/rumen_only/wuf_bodysitespeciesspecific_rumenonly.pdf",width=6,height=4.75); pcoa.w = plot_unifrac(otu.s,map,tree,category='BodysiteSpeciesSpecific',weight=T); 
graphics.off()
adonis(pcoa.u ~ map$BodysiteSpeciesSpecific)       # Do stats for clustering (unweighted)
adonis(pcoa.w ~ map$BodysiteSpeciesSpecific)       # Do stats for clustering (weighted)
adonis(bray ~ map$BodysiteSpeciesSpecific)         # Do stats for bray-curtis too, why not
...
plot(div.isimp~map$BodysiteSpeciesSpecific, xlab="Body Site",ylab="Shannon Diversity")
plot(div.shannon~map$BodysiteSpeciesSpecific, xlab="Body Site",ylab="Inverse Simpson Diversity")
plot(rowSums(otu.r > 0) ~ map$BodysiteSpeciesSpecific, xlab="Body Site",ylab="Number of OTUs")

## old alpha ##
library(ggsignif)
otu.ad = data.frame(Div=div.shannon, Body_site=map$BodysiteSpeciesSpecific)
grps = levels(map$BodysiteSpeciesSpecific)
lab = "Alpha Diversity (Shannon)" #paste0("Alpha Diversity (",dix,")")
pdf(paste0("results/douc_ruminal_acidosis/rumen_only/AlphaDiv_bodysitespeciesspecific_rumenonly",dix,".pdf"),width=6,height=5.5)
...
dev.off()
otu.ad = data.frame(Div=rowSums(otu.r > 0), Body_site=map$BodysiteSpeciesSpecific)
grps = levels(map$BodysiteSpeciesSpecific)
lab = "Alpha Diversity (Number of OTUs)" 
pdf(paste0("results/douc_ruminal_acidosis/rumen_only/numOTU_bodysitespeciesspecific_rumenonly.pdf"),width=6,height=5.5)
plot(ggplot(otu.ad,aes(x=Body_site,y=Div,fill=Body_site)) + ylab(lab) + xlab("Body Site") + geom_violin(alpha=0.3) + 
       geom_signif(comparisons = list(grps[c(1,2)]), test='t.test', map_signif_level = T) + 
       geom_jitter(aes(color=Body_site),position=position_jitter(0.2),size=2)  )
dev.off()
...
lscolors = c("red","green","yellow","orange")
for (L in 1:length(bT)) {
  ...
  select = rowSums(otu.t > 0) > min(table(map$BodysiteSpeciesSpecific))/2 # reasonable general min
  ...
  ntax = nrow(otu.t)
  BS = map$BodysiteSpeciesSpecific
  ...
  for (m.ix in 1:ntax) {  # Loop through all the rows (taxa)
    try({ # Because some correlations may be inadmissable
      ps = polyserial(otu.t[m.ix,],map$BodysiteSpeciesSpecific,ML=T,std.err = T)
    ...
    KW.Pvals[m.ix] = kruskal.test(otu.t[m.ix,] ~ map$BodysiteSpeciesSpecific)$p.val
  }
  
  ## Taxa barplots -- Top 15 most abundant (kruskal sig. + other?)
  otu.m = otu.n
  otu.m = sweep(sqrt(otu.m),2,colSums(sqrt(otu.m)),'/')
  meanAb = apply(otu.m,1,FUN=function(x) tapply(x, map$BodysiteSpeciesSpecific, mean)) # group mean
  ...
  otu.m = merge(otu.m, map[,c("SampleID","BodysiteSpeciesSpecific")], by="SampleID")
  otu.m$Taxa = factor(otu.m$Taxa,levels=byAbundance,ordered=T) # add Taxa column
  
  ## Plot according to BodysiteSpeciesSpecific, sorted by abundance
  pdf(paste0("results/douc_ruminal_acidosis/rumen_only/TaxaSummary_rumenonly_L",bT[L],".pdf"),width = 8,height=7) # Make room for legend
  plot(ggplot(otu.m, aes(x = BodysiteSpeciesSpecific, y = RelativeAbundance, fill = Taxa)) +
         geom_bar(stat ="identity", position="fill") + labs(x="BodysiteSpeciesSpecific",y="Root Relative Abundance") +
  ...
  pdf(paste0("results/douc_ruminal_acidosis/rumen_only/TaxaSwarms_rumenonly_L",bT[L],".pdf"),width = 6.5,height=6.5)
  sink(paste0("results/douc_ruminal_acidosis/rumen_only/Taxa_Significance_rumenonly_L",bT[L],".txt"))   # Get ready to write the significant ones
  cat("Taxon\tPolyserial_Q\tPolyserial_Cor\tBodysiteSpeciesSpecific_Q\n")  # Print header
  if (num_sig) for (i in 1:num_sig) {
    taxon = rownames(res)[i]
    cat(res[taxon,]$short,'\t',res$Grp.Pvals[i],'\t',-res$Grp.Corrs[i],'\t',res$KW.Pvals[i],'\t','\n',sep='')
    beeswarm(otu.t[taxon,] ~ map$BodysiteSpeciesSpecific, xlab="Body Site",ylab="CLR Relative Abundance",main=res[taxon,]$short,
             col=alpha(lscolors,0.7),
             cex.axis=1.1,cex.main=1,cex=1.1,corral="random",pch=19)
    bxplot(otu.t[taxon,] ~ map$BodysiteSpeciesSpecific, add = TRUE)
  }
  sink(NULL)
  dev.off()
  ...
  gl = map$BodysiteSpeciesSpecific
  glpos = c(grep("Douc Foregut",gl),grep("Cow Rumen Control",gl),grep("Cow Rumen APSC",gl),grep("Cow Rumen GBSC",gl))
  gl = gl[glpos]
  ...
  levels(gl)= c("red","green","yellow","orange") # lscolors
  png(paste0("results/douc_ruminal_acidosis/rumen_only/Taxa_heatmap_rumenonly_L",bT[L],".png"),  # create PNG for the heat map        
